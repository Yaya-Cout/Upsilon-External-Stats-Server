...
class SuccessfulExternalInstallDaySerializer(serializers.HyperlinkedModelSerializer):
    class Meta:
        model = SuccessfulExternalInstallDay
        fields = ['applications', 'installations', 'wallpaper_count', 'date', 'id']
    ...
    def create(self, validated_data):
        ...
        try:
            # This code is broken if we go back in the time, but it shouldn't
            # happen in production
            obj = SuccessfulExternalInstallDay.objects.latest('date')
            if obj.date == date:
                data = {
                    "applications": obj.applications,
                    "installations": obj.installations,
                ...
                create_new = True
        except SuccessfulExternalInstallDay.DoesNotExist:
            create_new = True
        ...
        # HACK: To prevent information leak, we are returning data from the day
        # before instead of searching for a real fix for this problem
        return SuccessfulExternalInstallDay.objects.filter(~Q(date=date)).latest('date')
